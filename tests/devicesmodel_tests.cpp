...
TEST(DeviceModelTest, shouldFillFromDbWhenCreated)
{
...
TEST(DeviceModelTest, shouldAddDevice)
{
    ...
    EXPECT_CALL(connection, createInsertionQuery(QString("device"), ::testing::_))
    ...
    EXPECT_CALL(connection, createSelectQuery(QString("device"), QString("max(id) AS id")))
...
TEST(DeviceModelTest, shouldDeleteDeviceFromDb)
{
    QueryMock* queryMock = new QueryMock();
    ...
    EXPECT_CALL(connection, createDeleteQuery(QString("device"), parameters))
        .WillOnce((::testing::Return(::testing::ByMove(std::move(q2)))));

    EXPECT_CALL(*secondQueryMock, execute());
    model.deleteDevice(model.indexById(1));
}
